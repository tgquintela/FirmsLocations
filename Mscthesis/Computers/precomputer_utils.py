...
from ..IO.io_standarized import get_locations

from precomputers import general_pfeatures_computation
...
class PrecomputerCollection(Processer):
    ...
    def __init__(self, logfile, pathdata):
        self.precomputer_locations =\
            Precomputer_locations(logfile, pathdata)
        self.precomputer_population =\
            Precomputer_population(logfile, pathdata)
        self.precomputer_population_cp =\
            Precomputer_population_CP(logfile, pathdata)
        self.precomputer_network_cp =\
            Precomputer_network_cp(logfile, pathdata)
        self.precomputer_qvalues =\
            Precomputer_qvalues(logfile, pathdata)
        self.precomputer_neighnet =\
            Precomputer_neighnet(logfile, pathdata)
        self.precomputer_pfeatures =\
            Precomputer_pfeatures(logfile, pathdata)

    def precompute(self, pars_locs=[], pars_pop=[], pars_pop_cp=[],
                   pars_net_cp=[], pars_neighnet=[], pars_pfeatures=[],
                   pars_qval=[]):
...
class Precomputer_locations(Precomputer_specific):
    ...
    def _retrieve_i(self, namefile):
        key_firms, years, new_locs, methodvalues =\
    ...
    def __iter__(self):
        ...
        for namefile in listfiles:
            key_firms, years, new_locs, methodvalues =\
                read_locations(namefile)
            yield key_firms, years, new_locs, methodvalues
...
class Precomputer_population(Precomputer_specific):
    ...
    def _retrieve_i(self, namefile):
        key_firms, years, population_value, methodvalues =\
            read_population_assignation(namefile)
        return key_firms, population_value, methodvalues
    ...
    def __iter__(self):
        ...
        for namefile in listfiles:
            key_firms, years, population_value, methodvalues =\
                read_population_assignation(namefile)
            yield key_firms, population_value, methodvalues
...
class Precomputer_pfeatures(Precomputer_specific):
    ...
    def _retrieve_i(self, namefile):
        nif, year, pfeatures, methodvalues = read_pfeatures(namefile)
        return nif, year, pfeatures, methodvalues
    ...
    def __iter__(self):
        ...
        for namefile in listfiles:
            nif, year, pfeatures, methodvalues = read_pfeatures(namefile)
            yield nif, year, pfeatures, methodvalues
